...
from datacite.schema41 import dump_etree, tostring, validate, validator


def test_example_json_validates(example_json41):
    """Test the example file validates against the JSON schema."""
    validator.validate(example_json41)
...
def test_titles():
    ...
    elem = dump_etree({"titles": [{"title": "Test", "titleType": "Subtitle"}]}).xpath(
...
def test_resourcetype():
    """Test resource type."""
    elem = dump_etree({"resourceType": {"resourceTypeGeneral": "Software"}}).xpath(
    ...
    pytest.raises(
        KeyError, dump_etree, {"resourceType": {"resourceType": "Science Software"}}
    )
...
def test_alternateidentifiers():
    ...
    pytest.raises(TypeError, dump_etree, {"alternateIdentifiers": {"invalid": "data"}})
...
def test_formats():
    ...
    elem = dump_etree({"formats": ["abc"]}).xpath("/resource/formats/format")[0]
...
def test_version():
    ...
    elem = dump_etree({"version": "v3.1"}).xpath("/resource/version")[0]
            ...
            "geoLocations": [
                {
                    ...
                    "geoLocationPolygon": {
                        "polygonPoints": [
                            {"pointLongitude": 31.12, "pointLatitude": 67},
                            {"pointLongitude": 32, "pointLatitude": 68},
                            {"pointLongitude": 31.12, "pointLatitude": 67},
                        ]
                    },
                },
